...
def correction_step(x, P, R):
    H = np.eye(2)
    K = np.dot(np.dot(P, H.T), np.linalg.inv(np.dot(np.dot(H, P), H.T) + R)) #Kalman Gain
    x = x + np.dot(K, (np.dot(H, x) + np.random.normal(0, np.sqrt(R[0, 0]))))
