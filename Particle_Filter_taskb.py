...
def particle_filter_localization(particles, weights, estimated_state, covariance, z, u):
    noise_scale_factor = 1.0  # Increase this to make particles spread out more
    ...
    for i in range(num_particles):
        particles[:,i] = estimated_state.reshape(-1,) + noise_scale_factor * np.diag(covariance_sqrt) * \
...
def main():
    ...
    while simulation_time >= current_time:
        ...
        if show_animation:
            ...
            plt.scatter(particles[0, :], particles[1, :], color='g', s=10)
            ...
            ax_inset = plt.axes([0.7, 0.1, 0.2, 0.2])
            ax_inset.scatter(particles[0, :], particles[1, :], color='g', s=10)
